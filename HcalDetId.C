  ...
  if (!gen.null()) {
	...
	(subdet!=HcalBarrel && subdet!=HcalEndcap && 
	 ...
	 subdet!=HcalTriggerTower && subdet!=HcalOther)) {
      throw std::runtime_error("Invalid DetId"); // << "Cannot initialize HcalDetId from " << std::hex << gen.rawId() << std::dec; 
    }  
...
HcalDetId& HcalDetId::operator=(const DetId& gen) {
  if (!gen.null()) {
	...
	(subdet!=HcalBarrel && subdet!=HcalEndcap && 
	 subdet!=HcalOuter && subdet!=HcalForward &&
	 subdet!=HcalTriggerTower && subdet!=HcalOther)) {
      throw std::runtime_error("Invalid DetId"); // << "Cannot assign HcalDetId from " << std::hex << gen.rawId() << std::dec; 
    }
    id_ = newForm(gen.rawId());
  } else {
    id_ = gen.rawId();
